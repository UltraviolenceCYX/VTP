...
def main():
    ...
    print('=> Training...')
    with wandb.init(project="VTP", config=config, name="emb_256"):
        for epoch in range(start_epoch, end_epoch):
            print('Epoch: {}'.format(epoch))

            # lr_scheduler.step()
            train_3d(config, model, optimizer, train_loader, epoch, final_output_dir, writer_dict)
            precision, printRes = validate_3d(config, model, test_loader, final_output_dir)
