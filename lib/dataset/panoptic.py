...
class Panoptic(JointsDataset):
    def __init__(self, cfg, image_set, is_train, transform=None):
        ...
        self.limbs = LIMBS
        self.k = cfg.NETWORK.NUM_JOINTS
        """
        self.adjacent = np.zeros((self.k, self.k), dtype=np.float)
        for [n1, n2] in LIMBS:
            self.adjacent[n1][n2] = 1
            self.adjacent[n2][n1] = 1
        for idx in range(self.k):
            self.adjacent[idx][idx] = 1
    ...
    def __getitem__(self, idx):
        ...
        return input, target, weight, target_3d, meta, input_heatmap#, self.adjacent
